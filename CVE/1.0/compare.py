...
def get_data_from_dict():
    ...
    # 获取cve内容
    cve_dic_path = os.getcwd() + '/data/cve/'  # 存放的文件夹
    cve_dic_filename = 'cve_softname_and_version.txt'
    cve_dic_name = os.path.join('%s%s' % (cve_dic_path, cve_dic_filename))
    cve_dict = dict()  # 先初始化
    with open(cve_dic_name, 'r', encoding='UTF-8') as cve_f:  # 打开文件
        cve_lines = cve_f.readlines()  # 获取文件的所有行
        for cve_line in cve_lines:
            cve_raw = cve_line
            cve_raw = cve_raw.lstrip('name_and_version_dict=')
            cve_dict = ast.literal_eval(cve_raw)  # 转化为字典，只有一行数据，可以不加break
    ...
    return cpe_dict, cve_dict, nvd_dict


# 合并cve和nvd的数据到一个dict中
def get_cve_and_nvd_soft_origin():
    cpe_dict, cve_dict, nvd_dict = get_data_from_dict()
    cve_and_nvd_soft_origin = dict()

    cveid_cnt = 0
    for cveid in cve_dict:
        cveid_cnt = cveid_cnt + 1
        print('CVEID：' + str(cveid) + '    CVEID的数量：' + str(cveid_cnt) + '\n')
        cve_and_nvd_soft_origin[cveid] = {}
        cve_and_nvd_soft_origin[cveid]['cve'] = cve_dict[cveid]
        if cveid in nvd_dict:
            cve_and_nvd_soft_origin[cveid]['nvd'] = nvd_dict[cveid]
        else:
            cve_and_nvd_soft_origin[cveid]['nvd'] = ''  # 如果CVEID不存在于NVD中，则值为''
    # 保存在本地
    cve_and_nvd_soft_path = os.getcwd() + '/data/softname_version_compare/'  # 文件夹
    cve_and_nvd_soft_filename = '1_origin.txt'
    ...
    with open(cve_and_nvd_soft_file_path, 'w', encoding='utf-8') as name_and_version_f:  # 用w而非a，用于覆盖
        name_and_version_f.write('name_and_version_dict=' + str(cve_and_nvd_soft_origin))  # 写入数据
    # 为了方便观察，以较好的格式另外保存到一个文件中
    for j in cve_and_nvd_soft_origin:
        with open(print_name_and_version_file_path, 'a', encoding='utf-8') as print_name_and_version_f:  # 用a，用于追加
            print_name_and_version_f.write(j + ' ' + str(cve_and_nvd_soft_origin[j]) + '\n')  # 写入数据


# 在匹配软件名称前先预处理
# !!!!!!!!!!注意，如果后面要对比版本，需要把before等词语映射为<=符号
def clean_cve_and_nvd_softname_version():
    ...
    with open(soft_dic_name, 'r', encoding='UTF-8') as soft_f:  # 打开文件
        ...
        for soft_line in soft_lines:
            ...
            for cveid in soft_dict:
                ...
                while tmp_z < len(cve_list):  # 修改了CNNVD函数的for循环代码为while
                    # 处理软件名称，把软件名转换为小写，并且把符号'-'改为空格
                    version = cve_soft[cve_list[tmp_z]]
                    del cve_soft[cve_list[tmp_z]]  # 删除原来的键值
                    cve_list[tmp_z] = cve_list[tmp_z].lower().replace('_', ' ')
                    # 处理版本
                    tmp_i = 0
                    ...
                    while tmp_j < len(version):
                        ...
                        tmp_j = tmp_j + 1
                    cve_soft[cve_list[tmp_z]] = version  # 增加修改后的键值
                    tmp_z = tmp_z + 1
    ...
    # 保存在本地
    cve_and_nvd_soft_path = os.getcwd() + '/data/softname_version_compare/'  # 文件夹
    cve_and_nvd_soft_filename = '2_clean_softname_version.txt'  # 这样命名方便查看
...
def keep_same_version_of_cve_and_nvd():
    # 获取softname和version内容
    soft_dic_path = os.getcwd() + '/data/softname_version_compare/'  # 存放的文件夹
    ...
    # 保存在本地
    cve_and_nvd_soft_path = os.getcwd() + '/data/softname_version_compare/'  # 文件夹
    cve_and_nvd_soft_filename = '3_keep_same_version_of_cve_and_nvd.txt'  # 这样命名方便查看
    print_name_and_version_filename = '3_keep_same_version_of_cve_and_nvd_print.txt'
    cve_and_nvd_soft_file_path = os.path.join('%s%s' % (cve_and_nvd_soft_path, cve_and_nvd_soft_filename))
    print_name_and_version_file_path = os.path.join('%s%s' % (cve_and_nvd_soft_path, print_name_and_version_filename))
    # 字典原始格式保存在本地
    with open(cve_and_nvd_soft_file_path, 'w', encoding='utf-8') as name_and_version_f:  # 用w而非a，用于覆盖
