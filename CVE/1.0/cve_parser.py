...
# 分隔出软件名和版本
def get_softname_and_version_of_cve():
    cve_data_path = os.getcwd() + '/data/cve/'  # 存放cve的文件夹
    cve_data_filename = 'simple_data_of_cve.txt'
    cve_data_file_path = os.path.join('%s\%s' % (cve_data_path, cve_data_filename))
    name_and_version_dict = dict()  # 初始化
    with open(cve_data_file_path, 'r', encoding='UTF-8') as cve_f:  # 打开文件
        cve_lines = cve_f.readlines()  # 获取文件的所有行
        for cve_line in cve_lines:
            raw_data = cve_line.split()  # 按空格作为分隔符号切割字符串
            cveid = raw_data[2]  # 获取cveid，cveid作为字典的键
            print(cveid, '\n')
            name = raw_data[0].replace('_', ' ')  # 对于师姐标记的re_data里的软件名称/版本，如果某个软件名称/版本是多个word组成的，就用_连接
            name = name.replace('**', ' ')  # CVE-2015-3744这类CVE的软件名称的word之间是**
    ...
    # 保存在本地
    name_and_version_filename = 'cve_' + 'softname_and_version.txt'
    print_name_and_version_filename = 'cve_' + 'softname_and_version_print.txt'
    name_and_version_file_path = os.path.join('%s\%s' % (cve_data_path, name_and_version_filename))
    print_name_and_version_file_path = os.path.join('%s\%s' % (cve_data_path, print_name_and_version_filename))
