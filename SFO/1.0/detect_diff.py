...
# 保留不相同软件名称的数据项
def keep_not_same_version_of_cnvd_and_nvd():
    ...
    soft_dict = dict()
    not_same_version_set = set()  # 如果对于某CVEID，CNVD和NVD如果软件名称不同的话，存储这个CVEID
    ...
    with open(soft_dic_name, 'r', encoding='UTF-8') as soft_f:  # 打开文件
        ...
        for soft_line in soft_lines:
            ...
            while tmp_a < len(soft_fulllist):
                ...
                cnvd_soft = soft_dict[soft_fulllist[tmp_a]]['cnvd']
                ...
                if isinstance(cnvd_soft, dict):
                    ...
                    while tmp_i < len(cnvd_list):
                        if cnvd_list[tmp_i] not in nvd_soft:
                            not_same_version_set.add(soft_fulllist[tmp_a])  # 该行原代码 del cnvd_soft[cnvd_list[tmp_i]]
                        tmp_i = tmp_i + 1
    ...
    cnvd_and_nvd_soft_filename = '3_keep_not_same_version_of_cnvd_and_nvd.txt'  # 这样命名方便查看
    print_name_and_version_filename = '3_keep_not_same_version_of_cnvd_and_nvd_print.txt'
