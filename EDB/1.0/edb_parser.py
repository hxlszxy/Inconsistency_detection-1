...
# 分隔出软件名和版本
def get_softname_and_version_of_edb():
    edb_data_path = os.getcwd() + '/data/edb/'  # 存放edb的文件夹
    edb_data_filename = 'simple_data_of_edb.txt'
    edb_data_file_path = os.path.join('%s\%s' % (edb_data_path, edb_data_filename))
    name_and_version_dict = dict()  # 初始化
    with open(edb_data_file_path, 'r', encoding='UTF-8') as edb_f:  # 打开文件
        edb_lines = edb_f.readlines()  # 获取文件的所有行
        for edb_line in edb_lines:
            raw_data = edb_line.split()  # 按空格作为分隔符号切割字符串
    ...
    # 保存在本地
    name_and_version_filename = 'edb_' + 'softname_and_version.txt'
    print_name_and_version_filename = 'edb_' + 'softname_and_version_print.txt'
    name_and_version_file_path = os.path.join('%s\%s' % (edb_data_path, name_and_version_filename))
    print_name_and_version_file_path = os.path.join('%s\%s' % (edb_data_path, print_name_and_version_filename))
