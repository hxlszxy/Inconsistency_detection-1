...
def get_data_from_dict():
    ...
    # 获取edb内容
    edb_dic_path = os.getcwd() + '/data/edb/'  # 存放的文件夹
    edb_dic_filename = 'edb_softname_and_version.txt'
    edb_dic_name = os.path.join('%s%s' % (edb_dic_path, edb_dic_filename))
    edb_dict = dict()  # 先初始化
    with open(edb_dic_name, 'r', encoding='UTF-8') as edb_f:  # 打开文件
        edb_lines = edb_f.readlines()  # 获取文件的所有行
        for edb_line in edb_lines:
            edb_raw = edb_line
            edb_raw = edb_raw.lstrip('name_and_version_dict=')
            edb_dict = ast.literal_eval(edb_raw)  # 转化为字典，只有一行数据，可以不加break
    ...
    return cpe_dict, edb_dict, nvd_dict


# 合并edb和nvd的数据到一个dict中
def get_edb_and_nvd_soft_origin():
    cpe_dict, edb_dict, nvd_dict = get_data_from_dict()
    edb_and_nvd_soft_origin = dict()

    cveid_cnt = 0
    for cveid in edb_dict:
        cveid_cnt = cveid_cnt + 1
        print('cveid：' + str(cveid) + '    cveid的数量：' + str(cveid_cnt) + '\n')
        edb_and_nvd_soft_origin[cveid] = {}
        edb_and_nvd_soft_origin[cveid]['edb'] = edb_dict[cveid]
        if cveid in nvd_dict:
            edb_and_nvd_soft_origin[cveid]['nvd'] = nvd_dict[cveid]
        else:
            edb_and_nvd_soft_origin[cveid]['nvd'] = ''  # 如果cveid不存在于NVD中，则值为''
    # 保存在本地
    edb_and_nvd_soft_path = os.getcwd() + '/data/softname_version_compare/'  # 文件夹
    edb_and_nvd_soft_filename = '1_origin.txt'
    print_name_and_version_filename = '1_origin_print.txt'
    edb_and_nvd_soft_file_path = os.path.join('%s%s' % (edb_and_nvd_soft_path, edb_and_nvd_soft_filename))
    print_name_and_version_file_path = os.path.join('%s%s' % (edb_and_nvd_soft_path, print_name_and_version_filename))
    # 字典原始格式保存在本地
    with open(edb_and_nvd_soft_file_path, 'w', encoding='utf-8') as name_and_version_f:  # 用w而非a，用于覆盖
        name_and_version_f.write('name_and_version_dict=' + str(edb_and_nvd_soft_origin))  # 写入数据
    # 为了方便观察，以较好的格式另外保存到一个文件中
    for j in edb_and_nvd_soft_origin:
        with open(print_name_and_version_file_path, 'a', encoding='utf-8') as print_name_and_version_f:  # 用a，用于追加
            print_name_and_version_f.write(j + ' ' + str(edb_and_nvd_soft_origin[j]) + '\n')  # 写入数据


# 在匹配软件名称前先预处理
# !!!!!!!!!!注意，如果后面要对比版本，需要把before等词语映射为<=符号
def clean_edb_and_nvd_softname_version():
    ...
    with open(soft_dic_name, 'r', encoding='UTF-8') as soft_f:  # 打开文件
        ...
        for soft_line in soft_lines:
            ...
            for cveid in soft_dict:
                cveid_cnt = cveid_cnt + 1
                print('cveid：' + str(cveid) + '    cveid的数量：' + str(cveid_cnt) + '\n')
                edb_soft = soft_dict[cveid]['edb']

                # 对edb进行处理
                tmp_z = 0
                edb_list = list(edb_soft.keys())
                while tmp_z < len(edb_list):  # 修改了CNNVD函数的for循环代码为while
                    # 处理软件名称，把软件名转换为小写，并且把符号'-'改为空格
                    version = edb_soft[edb_list[tmp_z]]
                    del edb_soft[edb_list[tmp_z]]  # 删除原来的键值
                    edb_list[tmp_z] = edb_list[tmp_z].lower().replace('_', ' ')
                    # 处理版本
                    tmp_i = 0
                    ...
                    while tmp_j < len(version):
                        match_result = re.match(r'[-_*]+', version[tmp_j])  # 这些符号至少出现一次，能匹配'--'，不能匹配'edb-2018-0001'
                        ...
                        tmp_j = tmp_j + 1
                    edb_soft[edb_list[tmp_z]] = version  # 增加修改后的键值
                    tmp_z = tmp_z + 1
    ...
    # 保存在本地
    edb_and_nvd_soft_path = os.getcwd() + '/data/softname_version_compare/'  # 文件夹
    edb_and_nvd_soft_filename = '2_clean_softname_version.txt'  # 这样命名方便查看
...
def keep_same_version_of_edb_and_nvd():
    # 获取softname和version内容
    soft_dic_path = os.getcwd() + '/data/softname_version_compare/'  # 存放的文件夹
    ...
    with open(soft_dic_name, 'r', encoding='UTF-8') as soft_f:  # 打开文件
        ...
        for soft_line in soft_lines:
            ...
            for cveid in soft_dict:
                ...
                tmp_i = 0
                # 只对字典处理，不对空的字符串处理
                if isinstance(edb_soft, dict):
                    edb_list = list(edb_soft.keys())
                    while tmp_i < len(edb_list):
                        if edb_list[tmp_i] not in nvd_soft:
                            del edb_soft[edb_list[tmp_i]]
                        tmp_i = tmp_i + 1
                ...
                if isinstance(nvd_soft, dict):
                    ...
                    while tmp_j < len(nvd_list):
                        if nvd_list[tmp_j] not in edb_soft:
    ...
    while tmp_k < len(soft_list):
        edb_soft = soft_dict[soft_list[tmp_k]]['edb']
        nvd_soft = soft_dict[soft_list[tmp_k]]['nvd']
        if (not edb_soft) or (nvd_soft == ''):  # 为空则去掉
            del soft_dict[soft_list[tmp_k]]
        tmp_k = tmp_k + 1

    # 保存在本地
    edb_and_nvd_soft_path = os.getcwd() + '/data/softname_version_compare/'  # 文件夹
    edb_and_nvd_soft_filename = '3_keep_same_version_of_edb_and_nvd.txt'  # 这样命名方便查看
    print_name_and_version_filename = '3_keep_same_version_of_edb_and_nvd_print.txt'
    edb_and_nvd_soft_file_path = os.path.join('%s%s' % (edb_and_nvd_soft_path, edb_and_nvd_soft_filename))
    print_name_and_version_file_path = os.path.join('%s%s' % (edb_and_nvd_soft_path, print_name_and_version_filename))
    # 字典原始格式保存在本地
    with open(edb_and_nvd_soft_file_path, 'w', encoding='utf-8') as name_and_version_f:  # 用w而非a，用于覆盖
