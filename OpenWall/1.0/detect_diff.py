...
# 保留不相同软件名称的数据项
def keep_not_same_version_of_openwall_and_nvd():
    ...
    soft_dict = dict()
    not_same_version_set = set()  # 如果对于某cveid，openwall和NVD如果软件名称不同的话，存储这个cveid
    ...
    with open(soft_dic_name, 'r', encoding='UTF-8') as soft_f:  # 打开文件
        ...
        for soft_line in soft_lines:
            ...
            while tmp_a < len(soft_fulllist):
                ...
                openwall_soft = soft_dict[soft_fulllist[tmp_a]]['openwall']
                ...
                if isinstance(openwall_soft, dict):
                    ...
                    while tmp_i < len(openwall_list):
                        if openwall_list[tmp_i] not in nvd_soft:
                            not_same_version_set.add(soft_fulllist[tmp_a])  # 该行原代码 del openwall_soft[openwall_list[tmp_i]]
                        tmp_i = tmp_i + 1
    ...
    openwall_and_nvd_soft_filename = '3_keep_not_same_version_of_openwall_and_nvd.txt'  # 这样命名方便查看
    print_name_and_version_filename = '3_keep_not_same_version_of_openwall_and_nvd_print.txt'
